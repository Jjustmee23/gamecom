...
# GameCom Ubuntu VPS Auto Deployment Script
# Specifiek voor Ubuntu 20.04+ VPS
...
if [[ $EUID -ne 0 ]]; then
   error "Dit script moet als root worden uitgevoerd. Gebruik: sudo $0"
fi

log "🚀 GameCom Ubuntu VPS Auto Deployment Script gestart"
log "Domain: $DOMAIN"
log "Email: $EMAIL"
...
# Docker installatie voor Ubuntu
...
# Nginx configuratie voor Ubuntu
...
# Docker Compose production configuratie voor Ubuntu
...
services:
  postgres:
    ...
    networks:
      - ${APP_NAME}-network
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U gamecom"]
      interval: 30s
      timeout: 10s
      retries: 3
...
# UFW Firewall configuratie voor Ubuntu
log "🔥 UFW Firewall configureren..."
ufw --force enable
...
log "✅ UFW Firewall geconfigureerd"
...
# Systeem optimalisatie voor Ubuntu
log "⚡ Systeem optimalisatie..."
# Swap file maken als het niet bestaat
if [ ! -f /swapfile ]; then
    ...
    log "✅ Swap file aangemaakt"
fi

# Finale informatie
log "🎉 Ubuntu VPS Deployment voltooid!"
echo -e "${BLUE}"
echo "=========================================="
echo "        GAMECOM UBUNTU VPS DEPLOYMENT"
echo "=========================================="
...
log "✅ Ubuntu VPS Deployment script voltooid!" 